...
//        	alert(this._series[1]._plotData);
            for (var i=0; i<this.numberTicks; i++){
                var min = new $.jsDate(this.min);
                tt = min.add(i*this.daTickInterval[0], this.daTickInterval[1]).getTime();
//                alert(tt);
                var ttnext = new $.jsDate(tt).add(this.daTickInterval[0], this.daTickInterval[1]).getTime();
                if(ttnext>this.max)
            	{
                	var t = new this.tickRenderer(this.tickOptions);
                    t.showLabel = false;
                ...
                var flag=false;
                for(var k=0;k<this._series[1]._plotData.length;k++)
            	{
                	if(tt==this._series[1]._plotData[k][0])
                		flag=true;
            	}
                if(!flag)
            	{
