...
@Component(value = "remainingTasksBean")
@Scope("session")
public class RemainingTasksBean {
	...
	private static final Logger LOGGER = Logger.getLogger(RemainingTasksBean.class);
	
	private StateLazyLoading lazyLoadingRemainingTask;
	private boolean remainRemainingTask = true;
	...
	public void init() 
	{
		defaultStateLoadingRemainingTask();
	...
	public void sortRemainingTask(){
		...
		lazyLoadingRemainingTask.getSorters().setField(dataField);
		if("none".equals(currentValueSortField.toLowerCase())){
			lazyLoadingRemainingTask.getSorters().setValue(LAZYSORTER_VALUE.ASC);
		}else if("asc".equals(currentValueSortField.toLowerCase())){
			lazyLoadingRemainingTask.getSorters().setValue(LAZYSORTER_VALUE.DESC);
	...
	public void searchRemainingTask(){
		loadRemainingTasks();
	}
	
	public void loadMoreRemainingTask(){
		if(remainRemainingTask){
			int totalRow = lazyLoadingRemainingTask.getLimit() + lazyLoadingRemainingTask.getStep();
			lazyLoadingRemainingTask.setLimit(totalRow);
			loadRemainingTasks();
		}
		
	}
	
	private void defaultStateLoadingRemainingTask() {
		
		lazyLoadingRemainingTask = new StateLazyLoading();
		lazyLoadingRemainingTask.setStart(0);
		lazyLoadingRemainingTask.setLimit(15);
		lazyLoadingRemainingTask.setStep(15);
		
		//default sort
		LazySorter lazySorterForRemainingTask = new LazySorter();
		lazySorterForRemainingTask.setField("dateCreate");
		lazySorterForRemainingTask.setValue(LAZYSORTER_VALUE.DESC);
		...
		lazyLoadingRemainingTask.setFilters(lazyFilterForUserProject);
		lazyLoadingRemainingTask.setSorters(lazySorterForRemainingTask);
	...
	private void loadRemainingTasks() {
		...
		//filter subject
		String dataSearch = String.valueOf(lazyLoadingRemainingTask.getFilters().getValue());
		if(!dataSearch.isEmpty()) {
			List<Issue> issuesAfterFilter = new ArrayList<Issue>();
			for(Issue issueItem : remainingTasks){
				if(issueItem.getSubject().toLowerCase().contains(dataSearch.toLowerCase())){
					issuesAfterFilter.add(issueItem);
				}
			}
			int limit = lazyLoadingRemainingTask.getLimit();
			if(limit > issuesAfterFilter.size()){
				limit = issuesAfterFilter.size();
			}
			int oldSizeRemainingTask = issuesAfterFilter.size();
			totalRowIssue = oldSizeRemainingTask;
			remainingTasks = issuesAfterFilter.subList(lazyLoadingRemainingTask.getStart(), limit);
			int newSizeRemainingTask = remainingTasks.size();
			if(oldSizeRemainingTask == newSizeRemainingTask){
				remainRemainingTask = false;
			}else{
				remainRemainingTask = true;
			}
		}else{
			int limit = lazyLoadingRemainingTask.getLimit();
			if(limit > remainingTasks.size()){
				limit = remainingTasks.size();
			}
			int oldSizeRemainingTask = remainingTasks.size();
			totalRowIssue = oldSizeRemainingTask;
			remainingTasks = remainingTasks.subList(lazyLoadingRemainingTask.getStart(), limit);
			int newSizeRemainingTask = remainingTasks.size();
			if(oldSizeRemainingTask == newSizeRemainingTask){
				remainRemainingTask = false;
			}else{
		...
		//sort remaining task
		List<Issue> onlyIssueParent = new ArrayList<Issue>();
		Map<Long, List<Issue>> childIssueOfParent = new HashMap<Long, List<Issue>>();
		childIssueOfParent.clear();
		for(Issue issueParent : remainingTasks){
			if(issueParent.getParent() == null){
				//list parent
				onlyIssueParent.add(issueParent);
				List<Issue> issueChild = new ArrayList<Issue>();
				for(Issue issueChildItem : remainingTasks){
					if(issueChildItem.getParent() != null && issueChildItem.getParent().getIssueId().compareTo(issueParent.getIssueId()) == 0){
						issueChild.add(issueChildItem);
					}
				}
				//add list child into hashmap
				childIssueOfParent.put(issueParent.getIssueId(), issueChild);
			}
		}
		//sort asc by subject
		if(lazyLoadingRemainingTask.getSorters().getValue().equals(LAZYSORTER_VALUE.ASC) && "subject".equals(lazyLoadingRemainingTask.getSorters().getField())){
			Comparator<Issue> subjectASC = new Comparator<Issue>() {
				@Override
				public int compare(Issue o1, Issue o2) {
					return o1.getSubject().compareTo(o2.getSubject());
				}
			};
			Collections.sort(onlyIssueParent,subjectASC);
			//create new list after sort
			//clear remain tasks
			remainingTasks.clear();
			for(Issue parentIssue : onlyIssueParent){
				remainingTasks.add(parentIssue);
				remainingTasks.addAll(childIssueOfParent.get(parentIssue.getIssueId()));
			}
			//sort asc by subject
		}else if(lazyLoadingRemainingTask.getSorters().getValue().equals(LAZYSORTER_VALUE.DESC) && "subject".equals(lazyLoadingRemainingTask.getSorters().getField())){
			Comparator<Issue> subjectDESC = new Comparator<Issue>() {
				@Override
				public int compare(Issue o1, Issue o2) {
					return o2.getSubject().compareTo(o1.getSubject());
				}
			};
			Collections.sort(onlyIssueParent,subjectDESC);
			//create new list after sort
			//clear remain tasks
			remainingTasks.clear();
			for(Issue parentIssue : onlyIssueParent){
		...
		}else if(lazyLoadingRemainingTask.getSorters().getValue().equals(LAZYSORTER_VALUE.DESC) && "status".equals(lazyLoadingRemainingTask.getSorters().getField())){
			Comparator<Issue> statusASC = new Comparator<Issue>() {
				@Override
				public int compare(Issue o1, Issue o2) {
					return o1.getStatus().getName().compareTo(o2.getStatus().getName());
				}
			};
			Collections.sort(onlyIssueParent,statusASC);
			//create new list after sort
			//clear remain tasks
			remainingTasks.clear();
			for(Issue parentIssue : onlyIssueParent){
		...
		else if(lazyLoadingRemainingTask.getSorters().getValue().equals(LAZYSORTER_VALUE.ASC) && "status".equals(lazyLoadingRemainingTask.getSorters().getField())){
			Comparator<Issue> statusDESC = new Comparator<Issue>() {
				@Override
				public int compare(Issue o1, Issue o2) {
					return o2.getStatus().getName().compareTo(o1.getStatus().getName());
				}
			};
			Collections.sort(onlyIssueParent,statusDESC);
			//create new list after sort
			//clear remain tasks
			remainingTasks.clear();
			for(Issue parentIssue : onlyIssueParent){
	...
	public void setVoid() {
		try {
			...
			userStoryToSetVoid.setPreviousStatus(userStoryToSetVoid.getStatus());
			userStoryToSetVoid.setStatus(UserStory.StatusType.VOID);
			userStoryService.update(userStoryToSetVoid);
//			issueService.deleteAllUnexpiredIssuesWhenSetUserStoryVoid(userStoryToSetVoid.getUserStoryId());
//			kanbanIssueService.deleteAllKanbanIssueByUserStoryID(userStoryToSetVoid.getUserStoryId());
			issueService.setVoidAllUnexpiredIssuesWhenSetUserStoryVoid(userStoryToSetVoid.getUserStoryId());
			kanbanIssueService.setVoidAllKanbanIssueWhenSetVoidUserStory(userStoryToSetVoid.getUserStoryId());
	...
	public StateLazyLoading getLazyLoadingRemainingTask() {
		return lazyLoadingRemainingTask;
	}

	public void setLazyLoadingRemainingTask(
			StateLazyLoading lazyLoadingRemainingTask) {
		this.lazyLoadingRemainingTask = lazyLoadingRemainingTask;
	}

	public boolean isRemainRemainingTask() {
		return remainRemainingTask;
	}

	public void setRemainRemainingTask(boolean remainRemainingTask) {
		this.remainRemainingTask = remainRemainingTask;
	}
	...
	{
	    List<Team> listTeamsForSelection = new ArrayList<Team>();
	    if (allTeamsForDropdownList != null && allTeamsForDropdownList.size() > 0)
	    {
		for (Team team : allTeamsForDropdownList) 
		{
	    ...
	    allTeamsForDropdownList = listTeamsForSelection;
	}
}
