...
@Component("productBacklogBeanV2")
@Scope("session")
public class ProductBacklogBeanV2 implements Serializable {

	private static final Logger LOGGER = Logger.getLogger(ProductBacklogBeanV2.class);
	...
	private UserStory selectedUserStoryForSetVoid;
	...
	private List<Issue> unexpiredRelatedIssues;
	private UserStory.StatusType tempStatus;
	private List<UserStory.StatusType> selectedFilter = new ArrayList<UserStory.StatusType>();
	...
	private List<Issue> relatedIssueSelectedUserStoryForSetVoid;
	private List<KanbanIssue> relatedKanbanIssueSelectedUserStoryForSetVoid;
	...
	private StateLazyLoading stateUserstoryList;
	private Boolean remainingDataUserStoty = true;
	...
	public void init() {
		...
		//init state userstory
		defaultStateLazyLoading();
	}
	...
	public void selectOrUnSelectAllUserStory(){
		if(selectedAllUserStory){
			...
			//select all
			selectedUserStories.clear();
			List<UserStory> allUserStory = this.userStoryService.loadAllLazyUserstoryProjectId(this.projectId,stateUserstoryList);
			selectedUserStories.addAll(allUserStory);
	...
	public void checkExportUserStoryExcel(){
		if(selectedUserStories.isEmpty()) {
			RequestContext.getCurrentInstance().execute("exportError.show()");
		}else {
			RequestContext.getCurrentInstance().execute("triggerClickExportExcel()");
		}
	}
		
	private void defaultStateLazyLoading() {
		//init status 
		selectedFilter.add(UserStory.StatusType.TODO);
		selectedFilter.add(UserStory.StatusType.IN_PROGRESS);
		
		stateUserstoryList = new StateLazyLoading();
		stateUserstoryList.setStart(0);
		stateUserstoryList.setLimit(15);
		stateUserstoryList.setStep(10);
		//default sort
		LazySorter lazySorterForUserstory = new LazySorter();
		lazySorterForUserstory.setField("value");
		lazySorterForUserstory.setValue(LAZYSORTER_VALUE.DESC);
		
		//default field
		LazyFilter lazyFilterForUserstory = new LazyFilter();
		List<String> fields = new ArrayList<String>();
		fields.add("name");
		fields.add("userStoryId");
		lazyFilterForUserstory.setField(fields);
		lazyFilterForUserstory.setValue("");
		//default in list
		...
		stateUserstoryList.setFilterInList(filterInListStatus);
		
		stateUserstoryList.setSorters(lazySorterForUserstory);
		stateUserstoryList.setFilters(lazyFilterForUserstory);
		
	}
	
	private void resetStateLoadingUserstory(){
		defaultStateLazyLoading();
	}
	
	public void loadMoreUserStory(){
		int currentLimit = stateUserstoryList.getLimit();
		stateUserstoryList.setLimit((currentLimit + stateUserstoryList.getStep()));
		updateUserStoryList();
	}
	
	public void filterUserStory(){
		selectedAllUserStory = false;
		selectedUserStories.clear();
		updateUserStoryList();
	}
	...
	public void filterStatusUserStory(){
		...
		if(selectedFilter != null && !selectedFilter.isEmpty()){
			...
			totalRowProductBacklog = 0;
			
		}
	}
	
	public void sortUserStory(){
		...
		stateUserstoryList.getSorters().setField(dataField);
		if("none".equals(currentValueSortField.toLowerCase())){
			stateUserstoryList.getSorters().setValue(LAZYSORTER_VALUE.ASC);
		}else if("asc".equals(currentValueSortField.toLowerCase())){
			stateUserstoryList.getSorters().setValue(LAZYSORTER_VALUE.DESC);
	...
	private void updateUserStoryList() {
		this.userStoryList = this.userStoryService.loadLazyUserstoryProjectId(this.projectId,stateUserstoryList);
		//check remain data here
		int totalRecord = this.userStoryService.countTotalRowloadLazyUserstoryProjectId(this.projectId,stateUserstoryList);
		totalRowProductBacklog = totalRecord;
		if(totalRecord == userStoryList.size()){
			remainingDataUserStoty = false;
		}else{
			remainingDataUserStoty = true;
		}
		if(selectedAllUserStory == true){
			selectedUserStories.clear();
	...
	public void exportExcelProductBacklogFromProject() throws WriteException, IOException{
		...
		List<UserStory> userStories = this.selectedUserStories;//this.userStoryService.findAllUserStoryByProjectId(this.projectId);
	...
	public void processSetVoidUserStory(){
		if(selectedUserStoryForSetVoid != null){
			Long userStoryId = selectedUserStoryForSetVoid.getUserStoryId();
			UserStory tempUserstory = userStoryService.findUserStoryById(userStoryId);
			tempUserstory.setPreviousStatus(tempUserstory.getStatus());
			tempUserstory.setStatus(UserStory.StatusType.VOID);
			if(userStoryService.update(tempUserstory)){
//				kanbanIssueService.deleteAllKanbanIssueByUserStoryID(userStodyId);
//				issueService.deleteAllUnexpiredIssuesWhenSetUserStoryVoid(userStodyId);
				issueService.setVoidAllUnexpiredIssuesWhenSetUserStoryVoid(userStoryId);
				kanbanIssueService.setVoidAllKanbanIssueWhenSetVoidUserStory(userStoryId);
			}
			updateUserStoryList();
			if (tempUserstory.getStatus() == UserStory.StatusType.VOID) {
				setOtherValueForCurrentUserStory();
				tempUserstory.setStatus(UserStory.StatusType.VOID);
			}
	...
	public void processSetDestroyVoid(){
		if(selectedUserStoryForSetVoid != null){
			Long userStodyId = selectedUserStoryForSetVoid.getUserStoryId();
			UserStory tempUserstory = userStoryService.findUserStoryById(userStodyId);
			setTempStatus(this.userStory.getStatus());
			tempUserstory.setStatus(tempUserstory.getPreviousStatus());
			if(userStoryService.update(tempUserstory)){
				userStoryService.updateAllIssueOfUserStoryHaveTheSameContent(tempUserstory);
				issueService.destroyVoidAllUnexpiredIssuesWhenDestroyVoidUserStory(tempUserstory.getUserStoryId());
				kanbanIssueService.destroyVoidAllKanbanIssueWhenDestroyVoidUserStory(tempUserstory.getUserStoryId());
			}
			updateUserStoryList();
			setOtherValueForCurrentUserStory();
		}
...
//	=================== SET VOID WHEN EDIT ===================
	...
	public void processSetVoidUserStoryEdit(){
		if(userStory != null){
			userStory.setPreviousStatus(userStory.getStatus());
			userStory.setStatus(UserStory.StatusType.VOID);
			if(userStoryService.update(userStory)){
//				kanbanIssueService.deleteAllKanbanIssueByUserStoryID(userStory.getUserStoryId());
//				issueService.deleteAllUnexpiredIssuesWhenSetUserStoryVoid(userStory.getUserStoryId());
				issueService.setVoidAllUnexpiredIssuesWhenSetUserStoryVoid(userStory.getUserStoryId());
				kanbanIssueService.setVoidAllKanbanIssueWhenSetVoidUserStory(userStory.getUserStoryId());
			}
			updateUserStoryList();
	...
	public void processSetDestroyVoidEdit(){
		if(userStory != null){
			userStory.setStatus(userStory.getPreviousStatus());
			if(userStoryService.update(userStory)){
				userStoryService.updateAllIssueOfUserStoryHaveTheSameContent(userStory);
				issueService.destroyVoidAllUnexpiredIssuesWhenDestroyVoidUserStory(userStory.getUserStoryId());
				kanbanIssueService.destroyVoidAllKanbanIssueWhenDestroyVoidUserStory(userStory.getUserStoryId());
			}
			updateUserStoryList();
			setOtherValueForCurrentUserStory();
		}
	...
	public void saveUserStory() {
		try {
			if (this.createMode) {
				...
				this.userStory.setDescription(this.userStory.getDescription().replaceAll("\\p{Cntrl}", ""));
				this.userStory.setNote(this.userStory.getNote().replaceAll("\\p{Cntrl}", ""));
				// this.userStory.setPreviousStatus(userStoryService.findUserStoryById(userStory.getUserStoryId()).getStatus());
				LOGGER.warn("update userstory:" + this.userStory.getUserStoryId() + ","+
						this.userStory.getName() +", priority:"+ this.userStory.getPriority() +", status:"+ this.userStory.getStatus() );
				if (this.userStoryService.update(this.userStory)) {
					if(this.userStory.getStatus() == UserStory.StatusType.VOID){
						kanbanIssueService.deleteAllKanbanIssueByUserStoryID(this.userStory.getUserStoryId());
//						issueService.deleteAllUnexpiredIssuesWhenSetUserStoryVoid(this.userStory.getUserStoryId());
						issueService.setVoidAllUnexpiredIssuesWhenSetUserStoryVoid(userStory.getUserStoryId());
	...
	public void viewProductBackLog() {
		try {
			...
			unexpiredRelatedIssues = issueService.findAllUnexpiredChildrenIssuesByUserStory(userStory.getUserStoryId());
			relatedKanbanIssues = kanbanIssueService.findKanbanIssuesByUserStory(userStory.getUserStoryId());
		} catch (Exception e) {
			e.printStackTrace();
			LOGGER.error("view product backlog error",e);
		}
		
	}

//	private void setRelatedIssueOfSelectedUserStory() {
//		try {
//			
//		} catch (Exception e) {
...
//	}
	...
	public void goToViewUserStoryPage() {
		setProjectId(this.userStory.getProject().getProjectId());
	...
	public void setRelatedIssueForSelectedUserStory(){
		relatedIssueSelectedUserStoryForSetVoid = issueService.findAllUnexpiredChildrenIssuesByUserStory(selectedUserStoryForSetVoid.getUserStoryId());
		relatedKanbanIssueSelectedUserStoryForSetVoid = kanbanIssueService.findKanbanIssuesByUserStory(selectedUserStoryForSetVoid.getUserStoryId());
	}
	...
	public StateLazyLoading getStateUserstoryList() {
		return stateUserstoryList;
	}


	public void setStateUserstoryList(StateLazyLoading stateUserstoryList) {
		this.stateUserstoryList = stateUserstoryList;
	}

	public Boolean getRemainingDataUserStoty() {
		return remainingDataUserStoty;
	}

	public void setRemainingDataUserStoty(Boolean remainingDataUserStoty) {
		this.remainingDataUserStoty = remainingDataUserStoty;
	}
	...
	public UserStory getSelectedUserStoryForSetVoid() {
		return selectedUserStoryForSetVoid;
	}

	public void setSelectedUserStoryForSetVoid(UserStory selectedUserStoryForSetVoid) {
		this.selectedUserStoryForSetVoid = selectedUserStoryForSetVoid;
	}
	...
	public void setRelatedKanbanIssues(List<KanbanIssue> relatedKanbanIssue) {
		this.relatedKanbanIssues = relatedKanbanIssue;
	}


	public List<Issue> getRelatedIssueSelectedUserStoryForSetVoid() {
		return relatedIssueSelectedUserStoryForSetVoid;
	}


	public void setRelatedIssueSelectedUserStoryForSetVoid(
			List<Issue> relatedIssueSelectedUserStoryForSetVoid) {
		this.relatedIssueSelectedUserStoryForSetVoid = relatedIssueSelectedUserStoryForSetVoid;
	}


	public List<KanbanIssue> getRelatedKanbanIssueSelectedUserStoryForSetVoid() {
		return relatedKanbanIssueSelectedUserStoryForSetVoid;
	}

	public void setRelatedKanbanIssueSelectedUserStoryForSetVoid(
			List<KanbanIssue> relatedKanbanIssueSelectedUserStoryForSetVoid) {
		this.relatedKanbanIssueSelectedUserStoryForSetVoid = relatedKanbanIssueSelectedUserStoryForSetVoid;
	}


	public List<Issue> getUnexpiredRelatedIssues() {
		return unexpiredRelatedIssues;
	}


	public void setUnexpiredRelatedIssues(List<Issue> unexpiredRelatedIssues) {
		this.unexpiredRelatedIssues = unexpiredRelatedIssues;
	}
